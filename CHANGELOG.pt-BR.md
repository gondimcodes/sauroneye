...
- **Detecção Dupla de Travas com Precisão `(Device, Inode)` (POSIX fcntl + flock)**: Atualizado o `PackageManagerChecker` para verificar simultaneamente travas BSD `flock(2)` e travas de registro POSIX `fcntl(F_SETLK)` nos arquivos canônicos (`/var/lib/dpkg/lock-frontend`, `/var/lib/dpkg/lock`, `/var/lib/rpm/.rpm.lock`, etc.). O `apt` e o `dpkg` no Debian utilizam travas POSIX que não eram interceptadas pela verificação isolada de `flock` durante comandos manuais como `apt full-upgrade`. A inspeção de `/proc/locks` valida rigorosamente `major:minor` do dispositivo (decomposição padrão `sys/sysmacros.h`) em conjunto com o `inode`, evitando falsos positivos por colisão entre sistemas de arquivos distintos.
- **Varredura Rápida de Processos em `/proc` para Instaladores Ativos**: Além dos arquivos de trava, o `PackageManagerChecker` inspeciona `/proc/[pid]/comm` para identificar instantaneamente processos instaladores ativos (`apt`, `apt-get`, `dpkg`, `rpm`, `dnf`, etc.), excluindo daemons passivos de segundo plano (`unattended-upgr`) para prevenir a supressão permanente de alertas FIM.
