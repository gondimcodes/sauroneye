...
- **POSIX Record Lock & flock Dual-Check with `(Device, Inode)` Precision**: Upgraded `PackageManagerChecker` to detect both BSD `flock(2)` and POSIX `fcntl(F_SETLK)` record locks on canonical package manager lock files (`/var/lib/dpkg/lock-frontend`, `/var/lib/dpkg/lock`, `/var/lib/rpm/.rpm.lock`, etc.). Debian's `apt` and `dpkg` acquire POSIX locks that previously escaped detection by BSD `flock` during manual commands such as `apt full-upgrade`. Inspects `/proc/locks` matching both device `major:minor` (standard `sys/sysmacros.h` hex decomposition) and inode number to prevent cross-filesystem false lock collisions.
- **Fast `/proc` Process Scanning Fallback for Active Installers**: In addition to lock file inspection, `PackageManagerChecker` scans `/proc/[pid]/comm` to instantly detect running foreground/worker installer binaries (`apt`, `apt-get`, `dpkg`, `rpm`, `dnf`, etc.), filtering out passive background daemons (`unattended-upgr`) to prevent permanent alert suppression.
